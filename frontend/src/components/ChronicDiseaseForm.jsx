...
export const ChronicDiseaseForm = ({ patientId, onClose, onSubmitSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.name || !formData.diagnosedDate) {
    ...
    setLoading(true);
    try {
      await patientApi.addChronicDisease(patientId, formData);
      onSubmitSuccess();
      onClose();
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to add condition. Please check connectivity.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white w-full max-w-md rounded-2xl border border-slate-100 shadow-xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-outfit font-bold text-slate-800 text-base">Add Chronic Condition</h3>
          </div>
